...
export default {
  ...
  computed: {
    ...
    totalVotesFormatted(){
      return this.totalVotes === 0 || this.totalVotes === 1 ? this.$t('activity.poll.single.vote.label',{0: this.totalVotes}) : this.$t('activity.poll.many.votes.label',{0: this.totalVotes});
    },
    mostVotes() {
      let max = 0;
      this.answers.filter(answer => {
